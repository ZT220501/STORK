...
class FlowMatchRKGScheduler_1stDerivative(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        if self._step_index == 0:
            ...
            velocity_derivative = (self.velocity_predictions[-1] - model_output) / (h1)
            velocity_second_derivative = None
            velocity_third_derivative = None
            
            self.velocity_predictions.append(model_output)
            velocity_approx_order = 1
            self._step_index += 1
        ...
        for j in range(1, self.s + 1):
            ...
            if j > 1:
                if j == 2:
                    fraction = 6 / ((self.s + 4) * (self.s - 1))
                else:
                    fraction = ((j + 3) * (j - 2)) / ((self.s + 4) * (self.s - 1))
    ...
    def velocity_approximation(self, velocity_approx_order, diff, model_output, velocity_derivative, velocity_second_derivative, velocity_third_derivative=None):
        if velocity_approx_order == 1:
            velocity = model_output + diff * velocity_derivative
        elif velocity_approx_order == 2:
