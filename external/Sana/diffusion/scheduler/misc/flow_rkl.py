...
class FlowMatchRKLScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        #print("="* 50)
        #print(f"model_output", model_output.dtype, model_output.shape)
        old_dtype = model_output.dtype
        old_sample_dtype =sample.dtype
        ...
        for j in range(1, self.s + 1):
            ...
            if j == 1:
                mu_tilde = 4 / (3 * (self.s**2 + self.s - 2))
                ...
                if j == 2:
                    mu = (2 * j - 1) / j
                    nu = -(j - 1) / j
                    mu_tilde = mu * 4 / (self.s**2 + self.s - 2)
                    ...
                    mu_tilde = (2 * j - 1) * (j + 2) * (j - 1)**2 / (j * (j - 2) * (j + 1)**2) * 4 / (self.s**2 + self.s - 2)
        ...
        img_next = img_next.to(old_sample_dtype)
