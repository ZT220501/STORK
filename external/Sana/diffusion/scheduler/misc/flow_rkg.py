...
class FlowMatchRKGScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        # print("self.sigmas is " + str(self.sigmas))
        # print("self.dt_list is " + str(self.dt_list))
        ...
        if self._step_index == 0:
            ...
            img_next = sample - 0.75 * dt * model_output + 0.25 * dt * self.first_velocity
            self.velocity_predictions.append(model_output)
            self._step_index += 1
            img_next = img_next.to(model_output.dtype)
            return SchedulerOutput(prev_sample=img_next)
        ...
        for j in range(1, self.s + 1):
            ...
            if j == 1:
                ...
                #Y_j = Y_j_1 - (t - t_next) * torch.ones(n, device=sample.device) * mu_tilde * model_output
            else:
