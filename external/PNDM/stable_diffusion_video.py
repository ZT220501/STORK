from diffusers import FlowMatchEulerDiscreteScheduler, FlowMatchHeunDiscreteScheduler, \
    DPMSolverMultistepScheduler,HunyuanVideoPipeline, PipelineQuantizationConfig, UniPCMultistepScheduler
...
def load_jobs(job_path, save_folder, overwrite=True):
    if os.path.exists(save_folder):
        ...
        if overwrite:
            print(f"Overwriting {len(generated_keys)} generated videos.")
        else:
            print(f"Skipping {len(generated_keys)} generated videos.")
    else:
        print('No saved videos found. Generating all videos.')
        generated_keys = []
...
def sample(rank, jobs, model_id, device, num_inference_steps, scheduler, precision, height=512, width=512,
           guidance_scale=3.5, batch_size=10, save_dir="output", seed=0, stopping_eps=1e-2, num_frames=129):
    ...
    # Load the model
    if model_id == "hunyuanvideo-community/HunyuanVideo":
        # Quantize the model
        ...
        pipeline = HunyuanVideoPipeline.from_pretrained(model_id, torch_dtype=torch_dtype, quantization_config=pipeline_quant_config)
    else:
    ...
    # This is a video pipeline, so we set batch size to 1.
    batch_size = 1
    ...
    with torch.inference_mode():
        ...
        for start_idx in tqdm(range(0, total_jobs, batch_size), total=total_batches, unit="Batch", desc=f"Proc-{rank} sampling", position=rank):
            ...
            export_to_video(video, f"{save_dir}/{im_ids[0]}.mp4", fps=15)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_inference_steps', help='how many steps to use for inference. Not necessarily NFE', type=int, default=10)
    parser.add_argument('--num_frames', help='number of frames to generate', type=int, default=129)
    ...
    parser.add_argument('--height', help='image size to use', default=320, type=int)
    parser.add_argument('--width', help='image size to use', default=512, type=int)
