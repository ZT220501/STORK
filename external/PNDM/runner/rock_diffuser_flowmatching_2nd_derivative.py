...
class FlowMatchROCKScheduler_2ndDerivative(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        old_model_output_type = model_output.dtype
        ...
        if self._step_index == 0:
            ...
            dt = torch.ones(n, device=sample.device, dtype=self.dtype) * dt
            dt = dt.view(-1, 1, 1, 1)
            ...
            self._step_index += 1
            img_next = img_next.to(old_model_output_type)
            ...
            dt_previous = torch.ones(n, device=sample.device, dtype=self.dtype) * dt_previous
            ...
            t = self.sigmas[self._step_index]
            t_start = torch.ones(n, device=sample.device, dtype=self.dtype) * t
        ...
        for j in range(1, mdeg + 1):
            ...
            if j == 1:
                ...
                temp1 = -(t - t_next) * recf[mr + 2 * (j-2) + 1] * torch.ones(n, device=sample.device, dtype=self.dtype)
                temp3 = -recf[mr + 2 * (j-2) + 2] * torch.ones(n, device=sample.device, dtype=self.dtype)
                temp2 = torch.ones(n, device=sample.device, dtype=self.dtype) - temp3
        ...
        temp1 = -(t - t_next) * fpa[mz,0] * torch.ones(n, device=sample.device, dtype=self.dtype)
        diff = ci1 - t_start
        diff = diff.view(-1, 1, 1, 1)
        velocity = self.velocity_approximation(velocity_approx_order, diff, model_output, velocity_derivative, velocity_second_derivative, velocity_third_derivative)
        Y_j_1 = velocity
        ...
        ci2 = ci1 + temp1
        temp1 = -(t - t_next) * fpa[mz,1] * torch.ones(n, device=sample.device, dtype=self.dtype)
        temp2 = -(t - t_next) * fpa[mz,2] * torch.ones(n, device=sample.device, dtype=self.dtype)
        diff = ci2 - t_start
        diff = diff.view(-1, 1, 1, 1)
        velocity = self.velocity_approximation(velocity_approx_order, diff, model_output, velocity_derivative, velocity_second_derivative, velocity_third_derivative)
        Y_j_2 = velocity
        ...
        ci2 = ci1 + temp1 + temp2
        temp1 = -(t - t_next) * fpa[mz,3] * torch.ones(n, device=sample.device, dtype=self.dtype)
        temp2 = -(t - t_next) * fpa[mz,4] * torch.ones(n, device=sample.device, dtype=self.dtype)
        temp3 = -(t - t_next) * fpa[mz,5] * torch.ones(n, device=sample.device, dtype=self.dtype)
        diff = ci2 - t_start
        diff = diff.view(-1, 1, 1, 1)
        velocity = self.velocity_approximation(velocity_approx_order, diff, model_output, velocity_derivative, velocity_second_derivative, velocity_third_derivative)
        Y_j_3 = velocity
        ...
        temp1 = -(t - t_next) * fpb[mz,0] * torch.ones(n, device=sample.device, dtype=self.dtype)
        temp2 = -(t - t_next) * fpb[mz,1] * torch.ones(n, device=sample.device, dtype=self.dtype)
        temp3 = -(t - t_next) * fpb[mz,2] * torch.ones(n, device=sample.device, dtype=self.dtype)
        temp4 = -(t - t_next) * fpb[mz,3] * torch.ones(n, device=sample.device, dtype=self.dtype)
        diff = ci2 - t_start
        diff = diff.view(-1, 1, 1, 1)
        velocity = self.velocity_approximation(velocity_approx_order, diff, model_output, velocity_derivative, velocity_second_derivative, velocity_third_derivative)
        Y_j_4 = velocity
