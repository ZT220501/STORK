...
def drift_function(betas, total_step, t_eval, y_eval, noise):
    ...
    beta_t = beta_t.to('cuda')
    t_eval = t_eval.to('cuda')
...
class ROCKScheduler_sorted(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        **kwargs
        ...
        if self._step_index == 0:
            ...
            # Calculate the drift
            drift = drift_function(self.betas, self.config.num_train_timesteps, t, sample, noise_initial)

            img_next = sample - 0.5 * dt * drift
            self.drift_previous = drift
            ...
            img_next = sample - 0.75 * dt * drift + 0.25 * dt * self.drift_previous
            self.drift_previous = drift
            self._step_index += 1
            img_next = img_next.to(model_output.dtype)
            return SchedulerOutput(prev_sample=img_next)
            ...
            img_next = sample - 1.5 * dt * drift + 0.5 * dt * self.drift_previous
            self.drift_previous = drift
            self.noise_predictions.append(model_output)
            self._step_index += 1
            img_next = img_next.to(model_output.dtype)
            ...
            # noise_derivative = (-self.noise_predictions[-2] + 4 * self.noise_predictions[-1] - 3 * model_output) / (2 * h1)
            # noise_second_derivative = 2 / (h1 * h2 * (h1 + h2)) * (self.noise_predictions[-2] * h1 - self.noise_predictions[-1] * (h1 + h2) + model_output * h2)
