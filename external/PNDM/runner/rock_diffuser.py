...
class ROCKScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        **kwargs
        ...
        #model_output = model(img, t_start * (total_step - 1))
        ...
        dt = torch.ones(n, device=sample.device) * self.dt
        dt = dt.view(-1, 1, 1, 1)
        ...
        if self._step_index == 0:
            ...
            # SECOND RUN
            t_previous = torch.ones(n, device=sample.device) * self.timesteps[0] / 1000
            drift_previous = drift_function(self.betas, self.config.num_train_timesteps, t_previous, self.initial_sample, self.noise_predictions[-1])
            ...
            # FOURTH RUN
            # h = 0.5 * self.dt * torch.ones(n, device=sample.device)
            # h = h.view(-1, 1, 1, 1)
            h = 0.5 * dt
            # FIXME I just modified this
            ...
            # ALL ELSE
            # h = self.dt * torch.ones(n, device=sample.device)
            # h = h.view(-1, 1, 1, 1)
            # FIXME my is dt fixed, not dt_list[-1]?
            #h = dt
            h = self.dt_list[-1]
            ...
            # h1 = self.dt_list[-1]
            # h2 = h1 + self.dt_list[-2]
            # h3 = h2 + self.dt_list[-3]

            # noise_derivative = ((h2 * h3) * (self.noise_predictions[-1] - noise_initial) - (h1 * h3) * (self.noise_predictions[-2] - noise_initial) + (h1 * h2) * (self.noise_predictions[-3] - noise_initial)) / (h1 * h2 * h3)
            # noise_second_derivative = 2 * ((h2 + h3) * (self.noise_predictions[-1] - noise_initial) - (h1 + h3) * (self.noise_predictions[-2] - noise_initial) + (h1 + h2) * (self.noise_predictions[-3] - noise_initial)) / (h1 * h2 * h3)
            # noise_third_derivative = 6 * ((h2 - h3) * (self.noise_predictions[-1] - noise_initial) + (h3 - h1) * (self.noise_predictions[-2] - noise_initial) + (h1 - h2) * (self.noise_predictions[-3] - noise_initial)) / (h1 * h2 * h3)
        ...
        for j in range(1, mdeg + 1):
            ...
            if j == 1:
                ...
                # beta_t = beta_0 + ci1 * (beta_1 - beta_0)

                log_mean_coeff = (-0.25 * ci1 ** 2 * (beta_1 - beta_0) - 0.5 * ci1 * beta_0) * total_step
                # log_mean_coeff = (-0.25 * ci1 ** 2 * (beta_1 - beta_0) - 0.5 * ci1 * beta_0)
                std = torch.sqrt(1. - torch.exp(2. * log_mean_coeff))
    ...
    def _time_shift_exponential(self, mu, sigma, t):
        # print("timestep before shift is " + str(t))
        # print("Shifted time is " + str(math.exp(mu) / (math.exp(mu) + (1 / t - 1) ** sigma)))
